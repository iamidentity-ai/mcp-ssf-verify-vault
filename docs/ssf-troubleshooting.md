...
| Receiver logs `[deleteSessions] status=403` | SSF management OIDC app on IBM Verify is missing entitlements | Re-run `infra/verify/bootstrap-verify.ts` (idempotent — adds missing entitlements only). Or manually grant the five entitlements in the Verify Admin Console: `readUsersAndGroups`, `revokeUserSession`, `revokeAllUserSessions`, `readOidcOAuthGrants`, `readOidcOAuthConsents`. |
